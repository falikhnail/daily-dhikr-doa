...
export function AlWaqiahDialog({ defaultOpen = false }: { defaultOpen?: boolean }) {
        ...
        <DialogHeader className="border-b bg-hero p-6 text-primary-foreground">
          <DialogTitle className="flex items-baseline justify-between gap-3">
            <span className="font-display text-2xl">
              Surah {AL_WAQIAH_INFO.name}
              <span className="ml-2 text-sm font-normal text-primary-foreground/70">
                ({AL_WAQIAH_INFO.meaning})
              </span>
            </span>
            <span className="font-arabic text-3xl text-accent">{AL_WAQIAH_INFO.arabicName}</span>
          </DialogTitle>
          <p className="mt-2 text-xs text-primary-foreground/75">
            {AL_WAQIAH_INFO.virtue}
          </p>
        </DialogHeader>
        ...
        <ScrollArea className="max-h-[70vh]">
          <div className="space-y-4 p-5 sm:p-6">
            ...
            <p className="pt-4 text-center text-xs italic text-muted-foreground">
              Semoga Allah melapangkan rezeki yang halal dan berkah. Aamiin.
            </p>
          </div>
        </ScrollArea>
      </DialogContent>
